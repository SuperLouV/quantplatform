...
- 2026-05-06：处理 Claude 第二轮代码审查：修复 UI 内置 scheduler 完成判定，避免把账户/期权/AI/宏观等可选 supplemental 产物当作每日刷新完成条件；`MacroRiskService` 过滤 `nan/inf` 情绪数值；本地 JSON POST 请求增加 64KB 上限；抽取 shared supplemental 摘要函数；新增 scheduler 和 macro risk 单元测试。验证：`make check PYTHON=python3.13` 通过 56 个单元测试。审查中的 XSS 与 chat loading 判断不采纳：当前 Markdown 渲染已经对内容做 `escapeHtml`，发送按钮已有禁用和“处理中”状态。
- 2026-05-06：按“以后只生成一份综合每日报告”的需求重构日报输出：`DailyReportService` 现在同时写 `daily_*.json` 和 `daily_*.md`，JSON 采用 `daily_comprehensive_report_v1` 结构，整合市场状态、scanner、真实持仓基本面/资金流代理/机构持仓数据缺口/技术走势/情绪新闻、Longbridge 自选监控、期权策略建议、数据刷新状态和 AI 读取契约。`DailyRefreshService` 新增 `portfolio_strategy` 补充任务，收盘后准备包会把真实持仓/自选策略分析写入 summary，日报优先读取这些结构化产物。决策面板 AI 对话新增读取结构化日报 JSON。验证：`PYTHONPATH=src python -m compileall -q src scripts` 和 `test_decision_chat` 通过；当前 Python 3.11 环境缺 `pandas`，日报/scheduler 相关导入型测试仍无法运行。
