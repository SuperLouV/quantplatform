...
- `manual`：用户直接指定的标的
- `themes`：按主题维护的股票集合
- `index.nasdaq100`：纳斯达克 100 成分股
- `index.sp500`：标普 500 成分股，后续补充
- `hot`：高热度股票，后续可来自成交额、涨跌幅、新闻热度或人工维护
- `system.seed_symbols`：系统内置的种子标的，例如指数 ETF
- `ai`：未来 AI 自动判断候选的入口

这些来源会先合并去重，不会在来源阶段直接决定买卖。

第一版不做全美全市场扫描。股票推荐范围先收敛在 `NASDAQ 100`、`S&P 500`、高热度股票和用户自定义列表，等数据质量、指标、信号和报告闭环稳定后，再评估是否扩展到更大 universe。
