...
Covered call 的 `return_on_capital_pct / annualized_return_pct` 第一版按 100 股正股名义本金估算，用于比较权利金效率；它不是新增现金占用。

## 真实持仓建议

已新增 `make options-advice`：

- 读取 Longbridge 只读账户持仓
- 对每只美股持仓检查 covered call 和 cash-secured put
- 使用 yfinance 期权链作为第一版报价来源
- 输出 JSON + Markdown 到 `data/reports/options_advice/`
- 输出字段包含行权价、到期日、bid/ask/mid、权利金收入、资金占用、盈亏平衡、年化回报率、硬性不通过和风险提示

Cash-secured put 默认只使用保守现金，不把 margin buying power 当成现金。
...
解析截图 OCR 文本并用 yfinance 验证：

```bash
make option-screenshot OPTION_SCREENSHOT_ARGS="--text-file ocr.txt --symbol AAPL"
```
...
- 将截图解析结果接入 `make options-advice` 的附加输入
- 增加财报事件、IV 分位、成交量/open interest、价差宽度和组合集中度检查
